...
Write-Host "前端静态目录: web/dist (由 config/application.toml 的 static_dir 指定)" -ForegroundColor Gray
...
Write-Host "或直接运行: .\target\release\gbserver.exe" -ForegroundColor Gray
Write-Host "`n服务地址: http://0.0.0.0:18080 (需先启动 PostgreSQL，如: docker compose up -d，并配置 config/application.toml)" -ForegroundColor Yellow
