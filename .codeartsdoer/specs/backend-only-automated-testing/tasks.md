...
本文档规划GBServer后端自动化测试体系的详细实施任务，按照测试基础设施、单元测试、集成测试、端到端测试、性能测试和CI/CD集成的顺序组织，确保任务间的依赖关系清晰、执行顺序合理。

**核心原则**: 所有任务仅针对后端代码进行自动化测试，不包含前端界面测试。
...
**输出**:
- 更新后的Cargo.toml（仅包含后端测试依赖）
- .cargo/config.toml配置文件
- 测试工具安装脚本

**验收标准**:
- 所有后端测试依赖正确配置在dev-dependencies中
- cargo test命令可正常执行
- 测试工具（cargo-llvm-cov、cargo-nextest）可正常使用
- 不包含任何前端测试依赖（如webdriver、puppeteer等）

**实施内容**:
1. 在Cargo.toml中添加后端测试依赖：
...
4. 在Cargo.toml中添加exclude配置，排除前端目录
...
**输出**:
- tests/目录及子目录（仅包含后端测试）
- tests/common/测试辅助模块
- tests/fixtures/测试数据目录（仅包含后端数据）

**验收标准**:
- 目录结构符合设计文档要求
- tests/common模块可被其他测试引用
- tests/fixtures包含基础后端测试数据文件
- 不包含任何前端测试目录（ui/、e2e-frontend/、visual/等）

**实施内容**:
1. 创建tests目录结构：
   - tests/integration/（后端集成测试）
   - tests/e2e/（后端端到端测试）
   - tests/common/（后端测试辅助代码）
   - tests/fixtures/（后端测试数据）
...
**输出**:
- tests/common/env.rs（后端环境管理模块）
- tests/common/database.rs（后端数据库管理模块）
...
**输出**:
- tests/common/fixtures.rs（后端Fixture加载器）
- tests/common/generator.rs（后端数据生成器）
- tests/common/seeder.rs（后端数据库种子数据）

**验收标准**:
- FixtureLoader可正确加载后端JSON和文本fixtures
- TestDataGenerator可生成随机后端测试数据
- DatabaseSeeder可初始化后端数据库种子数据
- 不包含任何前端测试数据
...
### 1.5 Mock服务模块开发
**任务描述**: 开发后端ZLMediaKit和SIP的Mock服务
...
**输出**:
- tests/common/mock/zlm_mock.rs（后端ZLM Mock服务）
- tests/common/mock/sip_mock.rs（后端SIP Mock服务）
...
### 阶段六：后端测试验证（第8周）
完成任务8.1-8.3，验证所有测试仅针对后端代码。

---

## 质量检查点

1. **基础设施完成后**：验证后端测试环境可正常启动，测试数据可正常加载
2. **单元测试完成后**：验证后端代码覆盖率≥80%，所有测试通过
3. **集成测试完成后**：验证所有后端API和数据库操作测试通过
4. **E2E测试完成后**：验证完整后端业务流程测试通过
5. **CI/CD集成完成后**：验证自动化测试流程正常工作
6. **后端测试验证完成后**：确认所有测试仅针对后端代码，无前端测试
...
```bash
# 检查测试依赖（确保无前端测试工具）
grep -r "webdriver\|puppeteer\|selenium" Cargo.toml

# 检查测试目录（确保无前端测试目录）
ls tests/ | grep -v "integration\|e2e\|common\|fixtures\|performance"

# 检查测试数据（确保无前端测试数据）
ls tests/fixtures/ | grep -v "devices\|platforms\|users\|sip"
```
