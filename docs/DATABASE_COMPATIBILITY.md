...
- `Cargo.toml:7-11` — feature 定义（默认 sqlite）
- `src/db/mod.rs:42-72` — Pool 与 create_pool
- `src/lib.rs:24-85` — init_db_tables
- `src/db/device.rs` — `check_sqlite_device_limit` 设备上限检查
- `src/sip/server.rs:1115` — REGISTER 路径调用上限检查（仅 SQLite feature 生效）
- `database/init-postgresql-2.7.4.sql` — PG schema
- `database/init-mysql-2.7.4.sql` — MySQL schema
- `database/init-sqlite-2.7.4.sql` — SQLite schema
- `docker-compose.sqlite.yml` — SQLite profile（零 PG/Redis 依赖）
- `tests/integration/sqlite_compat.rs` — 4 个 SQLite 集成测试（schema 初始化 / CRUD / 上限 / 登录）

## 8. SQLite 设备数量上限（运行时保护）

SQLite 单写者锁 + 并发读仍 OK 的特性决定了其适用规模有限。GBServer 引入**设备上限运行时检查**，避免超出 SQLite 能力范围。
...
sqlite_max_devices = 500  # 默认 500；None = 不限
```

环境变量：`GBSERVER__DATABASE__SQLITE_MAX_DEVICES=500`

### 8.2 行为

- **新增设备**：当前 `gb_device` 总数 < 上限 → 允许；否则 → SIP REGISTER 返回 **503 Service Unavailable**
- **更新已有设备**（重注册）：始终允许（upsert 不增加行数）
- **PG/MySQL 后端**：完全忽略此字段（`#[cfg(feature = "sqlite")]` 编译期消除）

### 8.3 实现

- `src/db/device.rs` 新增 `check_sqlite_device_limit(pool, device_id, max_devices) -> Result<(), DeviceLimitError>`
- `src/db/device.rs` 新增 `device_exists(pool, device_id) -> Result<bool>`
- `src/db/device.rs` 新增错误类型 `DeviceLimitError { current, limit, device_id }`
- `src/sip/server.rs::handle_register` 在 `upsert_device` 之前调用，超限返回 503

### 8.4 错误响应

```
REGISTER from 34020000001110000301
ERROR: SQLite 设备数量已达上限 (500/500); 请迁移到 PostgreSQL/MySQL 后端。
...
|----------|------|
| < 200 设备 | SQLite 完全足够 |
| 200~500 设备 | SQLite 接近上限，关注写并发（位置/心跳高频时考虑缓存或批写） |
| > 500 设备 | 迁移到 PostgreSQL（`cargo run --no-default-features --features postgres`），数据用 `sqlite3 .dump` 导出后导入 PG |

详细部署判断见 [`DEPLOYMENT_GUIDE.md`](./DEPLOYMENT_GUIDE.md) §1 部署分级总览。