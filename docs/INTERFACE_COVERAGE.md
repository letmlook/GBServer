...
> **Current source of truth:** Phase 0 parity auditing is now generated by `scripts/parity-audit/extract-interface-coverage.js`. Review `docs/parity/interface-coverage-phase-0.md` first for backend route, official frontend API, local frontend, and page-route gaps against the selected reference baseline. The historical notes below remain useful context, but individual claims may be stale after later implementation.

## 当前实现的差距分析（历史说明）

日期：2026-04-30

本文档记录当前 Rust 后端相对参考 Java 实现“直接平替”的剩余差距。结论基于本仓库当前前端 API、`src/router.rs` 已挂载路由、`handlers`/`db` 实现状态、README 中历史说明，以及本轮代码阅读。

## 总体结论

当前项目已经具备参考实现的接口外形，前端大多数请求不会 404；设备、通道、用户、平台、推流、拉流代理、ZLM Hook、云录像、录像计划等也已有不同程度的真实实现。

但距离“直接平替生产级 Java 实现”仍有明显缺口，主要集中在：

- GB28181 SIP 信令链路尚未覆盖完整参考实现的行为。
...
当前已有注册、Catalog、INVITE、PTZ、部分 SUBSCRIBE/NOTIFY 处理，但仍不是完整参考实现的行为。
...
- Catalog 同步依赖设备在线运行态；重启后的同步任务、任务状态、进度、错误码与参考实现仍不一致。
...
- `/api/server/media_server/save/delete/load` 与参考实现的自动配置、Hook 设置、节点启停健康检测仍不完整。
- `on_server_started` 只记录日志，没有重置节点状态、流计数、重新配置 Hook。
- `on_stream_not_found` 的自动拉流逻辑是简化推测，不等价于参考实现按设备/通道查找并触发播放。
...
- 通道异常修复、父级/行政区规则与参考实现仍需比对。

### 8. 日志和审计不完整

`/api/log/list` 当前可以避免页面失败，但不是完整参考实现的操作日志/日志文件管理。

缺口：

- 缺少操作审计表、请求记录、用户行为记录。
- 实时日志、历史日志、日志文件下载/筛选与参考实现 仍不完整。
...
虽然接口外形大多兼容，但参考前端和第三方调用方通常依赖字段细节。
...
- 参考实现常见配置项没有全部映射到 `config/application.toml`。
- 没有完整迁移工具、数据兼容说明、Java 参考实现数据库升级兼容策略。
...
7. 做接口契约基准对比测试：用同一套前端和脚本分别打 Java 参考实现与 Rust 后端，对比字段、状态码和行为。

## 备注

README 中“已实现接口”章节有历史滞后描述：部分曾标注为占位的推流、拉流代理、云录像、录像计划已经有真实实现；也有部分看似成功的接口仍只是兼容响应。后续应以生成报告和当前代码现状为准；本文档保留为历史上下文，并在必要时同步更新。
