-- GBServer 测试种子数据（PostgreSQL 兼容）
-- 表名 / 列名按 data/gbserver.db (SQLite) 实际 schema 编写；
-- PG 在生产中也使用相同 schema（见 database/init-postgresql-2.7.4.sql）

INSERT INTO gb_user_role (id, name, authority, create_time, update_time) VALUES
  (1, 'admin', '0', '2026-08-23 20:00:00', '2026-08-23 20:00:00'),
  (2, 'user',  '1', '2026-08-23 20:00:00', '2026-08-23 20:00:00')
ON CONFLICT (id) DO NOTHING;

INSERT INTO gb_user (id, username, password, role_id, push_key, create_time, update_time) VALUES
...
INSERT INTO gb_common_region (id, device_id, name, parent_id, parent_device_id, create_time, update_time) VALUES
...
INSERT INTO gb_common_group (id, device_id, name, parent_id, parent_device_id, business_group, create_time, update_time) VALUES
...
INSERT INTO gb_device_channel (
  ...
  ('34020000001320000003', '测试 NVR-通道2',      'MockVendor', 'MOCK-NVR-200', 'Admin', '110105',
   ...
   '2026-08-23 20:00:00', '2026-08-23 20:00:00')
ON CONFLICT (gb_device_id) DO NOTHING;

INSERT INTO gb_media_server (
  ...
  ('zlmediakit-1', '127.0.0.1', '127.0.0.1', '127.0.0.1', '127.0.0.1',
   8080, 1935, 554,
   'EWHpCV2W2SYa8SnpE2whVlUUlJJKhaiw', 'zlm', true, true,
   '2026-08-23 20:00:00', '2026-08-23 20:00:00')
ON CONFLICT (id) DO NOTHING;

INSERT INTO gb_platform (
  ...
  (true, 'Mock-级联平台-1', '34020000002000000099', '3402000000', '127.0.0.1', 5062,
   '34020000001320000001', '127.0.0.1', '5060',
   'admin', 'admin123', '3600', '60', 'UDP',
   '340200', 'MockVendor', 'MOCK-UP', 'Beijing',
   true, 1, '2026-08-23 20:00:00', '2026-08-23 20:00:00')
ON CONFLICT (server_gb_id) DO NOTHING;

INSERT INTO gb_jt_terminal (
